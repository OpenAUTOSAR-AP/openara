...
class Result final
{
   ...
   Result(Result&& other) noexcept(
      std
      : is_nothrow_move_constructible<T>::value&& std::is_nothrow_move_constructible<E>::value);
    ...
    *
    * @return Optional<T> an Optional with the value, if present
    */
   Optional<T> Ok() const&;
    ...
    *
    * @return Optional<E> an Optional with the error, if present
    */
...
class Result<void, E> final
{
    ...
    * @return SEE_BLOW a new Result instance of the possibly transformed type
    */
   template <typename F>
   auto Bind(F&& f) const -> SEE_BLOW;
};
